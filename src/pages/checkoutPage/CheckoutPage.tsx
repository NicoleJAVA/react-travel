      ...
      <Spin spinning={loading}>
        <div className={styles["content-container"]}>
          <table className={styles["data-table"]}>
            ...
            <tbody>
              {!isUdemy && order &&
                Object.keys(order).map(o => order[o])
                  .map((item, i) => (

                    <tr key={`order-item-${i}`} className={styles["order-item"]}>
                      <td>{item.product.title}</td>
                      <td className={styles["text-end"]}>{item.qty}</td>
                      <td className={styles["text-end"]}>${item.final_total}</td>
                    </tr>
                  ))}
            ...
            <tfoot>
              <tr>
                <td colSpan={2} className={styles["order-total-label"]}>總計</td>
                {orderMeta && orderMeta.order && <td className={styles["order-total"]}>${orderMeta.order.total}</td>}
              </tr>
            </tfoot>
          </table>


          {orderUser && <table className={styles["data-table"]}>
            <tbody>
              <tr>
              ...
              <tr >
                ...
                <td>
                  {orderMeta.order.is_paid && <span className={styles["paid"]} >付款完成</span>}
                  {!orderMeta.order.is_paid && <span className={styles["not-paid"]} >尚未付款</span>}
                </td>
              </tr>
            </tbody>
            {/* <tfoot>
          ...
          <div className={styles["pay-btn-container"]}>
            {!orderMeta.order.is_paid && <div className={styles["pay-btn"]}
              onClick={() => onPayOrder()}>確認付款去</div>}
          </div>
        </div>
      </Spin>
    </MainLayout >
